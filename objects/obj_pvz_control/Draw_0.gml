...
if(!place_meeting(x,y, obj_pvz_turret)){
	...
	draw_text_transformed(x+85,y-98,"* "+string(turret_values.turret_update[0]),0.5,0.5,0);
	...
	with(instance_nearest(x,y,obj_pvz_turret)){
		if(bullet_damage < 3){
			...
			draw_text_transformed(x+45,y-98,"*"+string(obj_pvz_control.turret_values.weapon_update[bullet_damage]),0.5,0.5,0);
		...
		if(image_index < 2){
			...
			draw_text_transformed(x+45,y-58,"*"+string(obj_pvz_control.turret_values.turret_update[image_index+1]),0.5,0.5,0);
